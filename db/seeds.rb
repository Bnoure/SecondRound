...
file = URI.open(url)
barbie = Game.create!(title: 'Barbie fait la vaisselle', category: 'simlutaion réaliste', price: 55, console: gameboy_color, description: "La poupée Barbie doit se préparer pour son rendez-vous avec Ken. Mais ses habits préférés sont tous éparpillés dans différents niveaux.", year: 1980, condition: 'neuf', store_id: stores.sample.id)
...
file = URI.open(url)
rocket = Game.create!(title: 'Rocket league', category: 'Course', price: 15, console: switch, description: "", year: 1980, condition: 'neuf', store_id: stores.sample.id)
...
file = URI.open(url)
soulcalibur3 = Game.create!(title: 'Soul calibur 3', category: 'Bagarre', price: 87, console: ps1, description: "Les combattants sont tous dotés d'une arme blanche spécifique. Le jeu propose de nombreux modes : une histoire aux multiples embranchements pour chaque personnage, un éditeur de combattant, et un mode Chroniques de l'épée mêlant stratégie et combat dans lequel vous dirigez une petite équipe.", year: 2005, condition: 'neuf', store_id: stores.sample.id)
...
file = URI.open(url)
zeldabotw = Game.create!(title: 'The Legend of Zelda : Breath of the Wild', category: 'Action, Aventure', price: 50, console: switch, description: "The Legend of Zelda : Breath of the Wild est un jeu d'action-aventure sur Switch. Vous incarnez Link, un jeune garçon qui doit sauver le royaume d'Hyrule de la menace de Ganon. Pour cela, il doit traverser de vastes étendues et explorer des donjons. Le titre propose une réalisation graphique en cel-shading et un univers coloré.", year: 2017, condition: 'Parfait état', store_id: stores.sample.id)
...
file = URI.open(url)
zeldaot = Game.create!(title: 'The Legend of Zelda : Ocarina of Time', category: 'Action, Aventure', price: 30, console: gamecube, description: "The Legend of Zelda : Ocarina of Time est un jeu d'action-aventure sur Gamecube. Vous incarnez Link, un jeune garçon qui doit sauver le royaume d'Hyrule de la menace de Ganon. Pour cela, il doit traverser de vastes étendues et explorer des donjons. Le titre propose une réalisation graphique en cel-shading et un univers coloré.", year: 1998, condition: 'Bon état', store_id: stores.sample.id)
...
file = URI.open(url)
supersmashbrosmelee = Game.create!(title: 'Super Smash Bros. Melee', category: 'Combat', price: 20, console: gamecube, description: "Super Smash Bros. Melee sur Gamecube est un jeu de combat dans lequel vous incarnez l'un des 25 personnages de l'univers de Nintendo, tels que Mario, Link, Pikachu ou encore Donkey Kong. Le titre propose de nombreux modes de jeu et un gameplay à la fois simple et profond.", year: 2002, condition: 'Etat correct', store_id: stores.sample.id)
...
file = URI.open(url)
alexkidd = Game.create!(title: 'Alex Kidd in Miracle World', category: 'Plateforme', price: 15, console: master_system, description: "Alex Kidd in Miracle World est un jeu de plates-formes sur Master System. Vous incarnez Alex Kidd, un jeune garçon qui doit sauver le royaume de Radaxian en battant Janken, le roi du mal. Le jeu propose de nombreux niveaux et des combats contre des boss.", year: 1986, condition: 'Parfait état', store_id: stores.sample.id)
